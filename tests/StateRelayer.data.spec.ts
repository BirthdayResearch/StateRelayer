...
describe("State relayer contract data tests", () => {
  ...
  describe("Successful", () => {
    it("Should successfully set master node data", async () => {
      ...
      await stateRelayerProxy
        .connect(bot)
        .updateMasterNodeInformation(masterNodeData);
      const receivedMasterNodeData =
        await stateRelayerProxy.masterNodeInformation();
      expect(receivedMasterNodeData.tvl).to.equal(masterNodeData.tvl);
      expect(receivedMasterNodeData.zeroYearTVL).to.equal(
        masterNodeData.zeroYearTVL
      );
      expect(receivedMasterNodeData.fiveYearTVL).to.equal(
        masterNodeData.fiveYearTVL
      );
      expect(receivedMasterNodeData.tenYearTVL).to.equal(
        masterNodeData.tenYearTVL
      );
      expect(receivedMasterNodeData.lastUpdated).to.equal(
        masterNodeData.lastUpdated
      );
    });
    ...
    it("Should successfully set vault node data", async () => {
      ...
      await stateRelayerProxy
        .connect(bot)
        .updateVaultGeneralInformation(vaultInformationData);
      const receivedMasterNodeData = await stateRelayerProxy.vaultInfo();
      expect(receivedMasterNodeData.noOfVaults).to.equal(
        vaultInformationData.noOfVaults
      );
      expect(receivedMasterNodeData.totalLoanValue).to.equal(
        vaultInformationData.totalLoanValue
      );
      ...
      expect(receivedMasterNodeData.activeAuctions).to.equal(
        vaultInformationData.activeAuctions
      );
      expect(receivedMasterNodeData.lastUpdated).to.equal(
        vaultInformationData.lastUpdated
      );
    });
    ...
    it("Should successfully set dexs data", async () => {
      ...
      const receivedEThDexData = await stateRelayerProxy.DEXInfoMapping(
        symbols[0]
      );
      // Testing that the received is as expected as dexDataEth
      expect(receivedEThDexData.primaryTokenPrice).to.equal(
        dexDataEth.primaryTokenPrice
      );
      expect(receivedEThDexData.volume24H).to.equal(dexDataEth.volume24H);
      expect(receivedEThDexData.totalLiquidity).to.equal(
        dexDataEth.totalLiquidity
      );
      expect(receivedEThDexData.APR).to.equal(dexDataEth.APR);
      expect(receivedEThDexData.firstTokenBalance).to.equal(
        dexDataEth.firstTokenBalance
      );
      expect(receivedEThDexData.secondTokenBalance).to.equal(
        dexDataEth.secondTokenBalance
      );
      ...
      const receivedBtcDexData = await stateRelayerProxy.DEXInfoMapping(
        symbols[1]
      );
      // Testing that the received is as expected as dexDataBtc
      expect(receivedBtcDexData.primaryTokenPrice).to.equal(
        dexDataBtc.primaryTokenPrice
      );
      expect(receivedBtcDexData.volume24H).to.equal(dexDataBtc.volume24H);
      expect(receivedBtcDexData.totalLiquidity).to.equal(
        dexDataBtc.totalLiquidity
      );
      expect(receivedBtcDexData.APR).to.equal(dexDataBtc.APR);
      expect(receivedBtcDexData.firstTokenBalance).to.equal(
        dexDataBtc.firstTokenBalance
      );
      expect(receivedBtcDexData.secondTokenBalance).to.equal(
        dexDataBtc.secondTokenBalance
      );
  ...
  describe("UnSuccessful", () => {
    ...
    it("`updateDEXInfo` - Should successfully revert if the signer is not `bot`", async () => {
      ...
      await expect(
        stateRelayerProxy.connect(user).updateDEXInfo(["eth"], [dexDataEth])
      ).to.be.reverted;
    });
  });
});
