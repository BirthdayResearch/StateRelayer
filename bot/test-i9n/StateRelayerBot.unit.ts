...
import {
  StateRelayer,
  StateRelayer__factory,
  StateRelayerProxy__factory,
} from "../../generated";
...
describe("State Relayer Bot Tests", () => {
  ...
  beforeEach(async () => {
    ...
    const stateRelayerImplementation =
      await hardhatNetwork.contracts.deployContract({
    ...
    const stateRelayerProxy = await hardhatNetwork.contracts.deployContract({
      ...
      deployArgs: [
        ...
        StateRelayer__factory.createInterface().encodeFunctionData(
          "initialize",
          [await admin.getAddress(), await bot.getAddress()]
        ),
      ],
    });
