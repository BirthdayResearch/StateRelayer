...
describe("HardhatNetwork", () => {
  describe("StartedHardhatNetworkContainer behavior", () => {
    ...
    it("should be able to connect via ethers", async () => {
      const provider = new ethers.providers.JsonRpcProvider(
        startedHardhatContainer.rpcUrl
      );
    ...
    it("should have some pre-funded accounts", async () => {
      const [preFundedAccount] = await startedHardhatContainer.call(
        "eth_accounts",
        []
      );
      const provider = new ethers.providers.JsonRpcProvider(
      ...
      expect(await provider.getBalance(preFundedAccount)).not.toStrictEqual(
        "0x0"
      );
    });
  });
  ...
  describe("HardhatNetwork behaviour", () => {
    ...
    it("should error if not generating at least 1 block", async () => {
      await expect(() => hardhatNetwork.generate(0)).rejects.toThrow(
        /Minimum of 1 block needs to be generated./
      );
      await expect(() => hardhatNetwork.generate(-1)).rejects.toThrow(
        /Minimum of 1 block needs to be generated./
      );
    ...
    it("should be able to create a funded test wallet", async () => {
      ...
      const testWalletBalance =
        await hardhatNetwork.ethersRpcProvider.getBalance(testWalletAddress);
      expect(testWalletBalance.eq(ethers.constants.MaxInt256)).toStrictEqual(
        true
      );
    });
    ...
    describe("getHardhatTestWallet", () => {
      it("should return a test wallet connected to a pre-initialised Hardhat account", async () => {
        await expect(
          hardhatNetwork.getHardhatTestWallet(0).testWalletSigner.getAddress()
        ).resolves.toStrictEqual(HardhatNetwork.hardhatAccounts[0]);
        await expect(
          hardhatNetwork.getHardhatTestWallet(19).testWalletSigner.getAddress()
      ...
      it("should throw an error when trying to get a test wallet for a Hardhat account that does not exist", async () => {
        expect(() => hardhatNetwork.getHardhatTestWallet(-1)).toThrow(
          "Please select an index from 0 to 19"
        );
        expect(() => hardhatNetwork.getHardhatTestWallet(20)).toThrow(
          "Please select an index from 0 to 19"
        );
      });
    });
  ...
  describe("Transaction auto-mining", () => {
    ...
    it("should auto-mine transactions when the TRANSACTION_AUTOMINE environment variable is true", async () => {
      startedHardhatContainer = await container
        .withEnvironment({ [TX_AUTOMINE_ENV_VAR]: "true" })
        .start();
      hardhatNetwork = await startedHardhatContainer.ready();
      ...
      const txReceipt =
        await hardhatNetwork.ethersRpcProvider.getTransactionReceipt(txHash);
