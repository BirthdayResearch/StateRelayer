...
export class HardhatNetwork {
  ...
  constructor(
    ...
    this.ethersRpcProvider = new ethers.providers.StaticJsonRpcProvider(
      startedHardhatContainer.rpcUrl
    );
    this.contractSigner = this.ethersRpcProvider.getSigner(
      contractDeployerAddress
    );
    this.contracts = new EvmContractManager(
  ...
  async stop(): Promise<void> {
    try {
      ...
      throw Error(
        "HardhatNetwork.stop stops the StartedHardhatNetworkContainer as well"
      );
    }
  ...
  async fundAddress(
    ...
    if (fundAccountStatus === false) {
      throw Error(
        "Something went wrong with funding the account with the specified amount"
      );
    }
  }
  ...
  async activateAccount(address: string): Promise<void> {
    return this.startedHardhatContainer.call("hardhat_impersonateAccount", [
      address,
    ]);
  }
  ...
  async generate(numBlocks: BigNumberish): Promise<void> {
    ...
    await this.startedHardhatContainer.call("hardhat_mine", [
      toZeroStrippedHex(numBlocks),
    ]);
  }
  ...
  getHardhatTestWallet(index: number): TestWalletData {
    ...
    if (testWalletAddress === undefined) {
      throw Error(
        "Please select an index from 0 to 19 (inclusive) when creating a hardhat wallet"
      );
    }
